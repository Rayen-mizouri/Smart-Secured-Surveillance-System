...
<body>
    ...
    def do_POST(self):
        ...
        # Note: Removed localhost redirect - it breaks remote client access
    ...
    def do_GET(self):
        """Handle GET requests"""
        # Note: Removed localhost redirect - it breaks remote client access
        # Remote clients accessing via IP/hostname should work directly without redirection
...
<body>
    ...
    <script>
        ...
        function playVideo(name, path) {
            ...
            player.onerror = function() {
                if (player.error) {
                    const errorMsg = {
                        ...
                        3: 'MEDIA_ERR_DECODE: The decoding process was aborted.',
                        4: 'MEDIA_ERR_SRC_NOT_SUPPORTED: The video format is not supported.'
                    };
                    alert('❌ ' + (errorMsg[player.error.code] || 'Unknown video error'));
                }
            };
            ...
            if (path.endsWith('.enc')) {
                ...
                downloadAndDecryptInBrowser(path, password, (blob, blobUrl) => {
                    player.src = blobUrl;
        ...
        async function downloadAndDecryptInBrowser(path, password, callback) {
            try {
                ...
                if (!decryptResponse.ok) {
                    ...
                    const contentType = decryptResponse.headers.get('content-type');
                    ...
                    try {
                        if (contentType && contentType.includes('application/json')) {
                            ...
                            errorMsg = errorText || `Server error (${decryptResponse.status})`;
                        }
                    } catch (e) {
                        errorMsg = `Server error (${decryptResponse.status}): ${decryptResponse.statusText}`;
                    }
        ...
        function downloadVideo(name, path) {
            ...
            if (path.endsWith('.enc')) {
                ...
                downloadAndDecryptInBrowser(path, password, (blob, blobUrl) => {
                    ...
                    link.href = blobUrl;
                    link.download = name.replace('.enc', '');
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
                    // Don't revoke URL immediately in case browser needs it
    ...
    def serve_video_file(self):
        ...
        try:
            ...
            if file_path.endswith('.enc'):
                ...
                print(f"📦 Serving encrypted: {file_name} ({file_size} bytes)")
                
                # Check for Range header
                if 'Range' in self.headers:
                    range_header = self.headers['Range']
                    print(f"⚠️ Range request detected: {range_header}")
                    # Don't support partial content for encrypted files - must be complete
                    print(f"   → Ignoring Range header, sending full file")
            else:
            ...
            # Send file without decryption (always full file)
            self.send_response(200)
            self.send_header("Content-type", content_type)
            self.send_header("Content-Length", str(file_size))
            self.send_header("Accept-Ranges", "none" if file_path.endswith('.enc') else "bytes")
            ...
            with open(file_path, 'rb') as f:
                ...
                if len(content) != file_size:
                    print(f"⚠️ File size mismatch: expected {file_size}, read {len(content)}")
    ...
    def handle_decrypt_request(self):
        ...
        try:
            ...
            # Read encrypted data - ensure we get all bytes
            encrypted_data = self.rfile.read(content_length)
            actual_length = len(encrypted_data)
            print(f"✅ Received {actual_length} bytes of encrypted data")
            
            # Verify we got all the data
            if actual_length != content_length:
                print(f"⚠️ Data size mismatch: expected {content_length}, got {actual_length}")
                self.send_response(400)
                self.send_header('Content-type', 'application/json')
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Data incomplete: expected {content_length} bytes, got {actual_length}"}).encode())
                return
            ...
            try:
                ...
                if len(encrypted_data) < 16:
                    raise ValueError(f"Encrypted data too short ({len(encrypted_data)} bytes), IV requires 16 bytes")
                
                # Check if data is multiple of 16 (AES block size) after IV
                ciphertext_length = len(encrypted_data) - 16
                if ciphertext_length % 16 != 0:
                    print(f"⚠️ Warning: Ciphertext length {ciphertext_length} is not multiple of 16")
                ...
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                self.wfile.write(json.dumps({"error": f"Decryption failed: {error_msg}"}).encode())
