...
class ClientDecryption:
    ...
    def decrypt_bytes_stream(self, encrypted_data: bytes) -> bytes:
        if len(encrypted_data) < self.IV_SIZE:
            raise ValueError(f"Invalid encrypted data: too short ({len(encrypted_data)} bytes, need at least {self.IV_SIZE})")
        ...
        if len(ciphertext) % 16 != 0:
            raise ValueError(f"Invalid ciphertext length: {len(ciphertext)} bytes is not multiple of 16 (AES block size)")
